...
bool WeatherManager::begin(){
  ...
  if(init){
    ...
    // Migrate the old placeholder into a temporary UI state. Never erase a
    // genuinely resolved city/region persisted for GPS.
    if(snapshot_.source==LocationSource::Gps&&!strcmp(snapshot_.city,"GPS LOCATION")){
      snapshot_.city[0]=0;preferences_.putString("city","");
    }
...
bool WeatherManager::resolveIdentity(Snapshot& n,double a,double o){
  // Reverse resolution is privacy-bounded: coordinates are sent only to the
  // provider, never rendered or logged, and only resolved labels are persisted.
  String url=String("https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=")+String(a,6)+
      "&longitude="+String(o,6)+"&localityLanguage=en";
  WiFiClientSecure client;client.setInsecure();HTTPClient http;
  http.setConnectTimeout(appconfig::kHttpConnectTimeoutMs);http.setTimeout(appconfig::kHttpReadTimeoutMs);
  bool resolved=false;
  if(http.begin(client,url)){
    const int status=http.GET();
    if(status>=200&&status<300&&http.getSize()<=(int)appconfig::kMaximumGeocodingResponseBytes){
      String body=http.getString();
      if(body.length()<=appconfig::kMaximumGeocodingResponseBytes){
        JsonDocument doc;
        if(!deserializeJson(doc,body)){
          const char* city=doc["city"]|"";
          if(!city[0])city=doc["locality"]|"";
          if(!city[0])city=doc["principalSubdivisionCode"]|"";
          const char* region=doc["principalSubdivision"]|"";
          const char* country=doc["countryName"]|"";
          const char* postal=doc["postcode"]|"";
          if(city&&city[0]){
            strlcpy(n.city,city,sizeof(n.city));strlcpy(n.region,region,sizeof(n.region));
            strlcpy(n.country,country,sizeof(n.country));strlcpy(n.postal,postal,sizeof(n.postal));
            resolved=true;
          }
        }
      }
  ...
  url=String();
  if(resolved)persistIdentity(n);
  return resolved;
}
void WeatherManager::persistIdentity(const Snapshot& n){
  persistResolvedIdentity(n);
}
void WeatherManager::persistResolvedIdentity(const Snapshot& n){
  preferences_.putString("city",n.city);preferences_.putString("region",n.region);
  preferences_.putString("country",n.country);preferences_.putString("postal",n.postal);
}
...
void WeatherManager::poll(uint32_t now){portENTER_CRITICAL(&controlMux_);if(suspended_){portEXIT_CRITICAL(&controlMux_);return;}inFlight_=true;bool manual=forceRefresh_;forceRefresh_=false;portEXIT_CRITICAL(&controlMux_);Snapshot n=snapshot();if(!n.configured||n.source==LocationSource::Disabled){portENTER_CRITICAL(&controlMux_);inFlight_=false;portEXIT_CRITICAL(&controlMux_);return;}double a=savedLatitude_,o=savedLongitude_;bool movedMaterially=false;if(n.source==LocationSource::Gps){if (!gpsValid_){n.state=State::GpsFixRequired;nextPollMs_=now+5000;publish(n);portENTER_CRITICAL(&controlMux_);inFlight_=false;portEXIT_CRITICAL(&controlMux_);return;}a=gpsLatitude_;o=gpsLongitude_;movedMaterially=lastRequestPositionValid_&&distanceKm(lastRequestLatitude_,lastRequestLongitude_,a,o)>=appconfig::kWeatherGpsMoveThresholdKm;if(!manual&&n.lastSuccessMs&&now-n.lastSuccessMs<appconfig::kWeatherPollIntervalMs&&!movedMaterially){nextPollMs_=n.lastSuccessMs+appconfig::kWeatherPollIntervalMs;portENTER_CRITICAL(&controlMux_);inFlight_=false;portEXIT_CRITICAL(&controlMux_);return;}}nextPollMs_=now+appconfig::kWeatherPollIntervalMs;if(WiFi.status()!=WL_CONNECTED){n.state=n.dataAvailable&&now-n.lastSuccessMs<=appconfig::kWeatherFreshForMs?State::Stale:State::Offline;publish(n);portENTER_CRITICAL(&controlMux_);inFlight_=false;portEXIT_CRITICAL(&controlMux_);return;}bool f=n.temperatureUnit==TemperatureUnit::Fahrenheit;String url=String("https://api.open-meteo.com/v1/forecast?latitude=")+String(a,6)+"&longitude="+String(o,6)+"&current=temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m,wind_direction_10m,precipitation_probability&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max&forecast_days=1&temperature_unit="+(f?"fahrenheit":"celsius")+"&wind_speed_unit=mph";WiFiClientSecure client;client.setInsecure();HTTPClient http;http.setConnectTimeout(appconfig::kHttpConnectTimeoutMs);http.setTimeout(appconfig::kHttpReadTimeoutMs);bool ok=false;if(http.begin(client,url)){int status=http.GET();if(status>=200&&status<300&&http.getSize()<=(int)appconfig::kMaximumWeatherResponseBytes){String body=http.getString();if(body.length()<=appconfig::kMaximumWeatherResponseBytes){JsonDocument doc;if(!deserializeJson(doc,body)){JsonObjectConst cur=doc["current"].as<JsonObjectConst>();double t=cur["temperature_2m"]|NAN,feels=cur["apparent_temperature"]|NAN,wind=cur["wind_speed_10m"]|NAN;int code=cur["weather_code"]|-1,humidity=cur["relative_humidity_2m"]|-1,direction=cur["wind_direction_10m"]|-1,precip=cur["precipitation_probability"]|-1;JsonObjectConst dailyObject=doc["daily"].as<JsonObjectConst>();double high=dailyObject["temperature_2m_max"][0]|NAN,low=dailyObject["temperature_2m_min"][0]|NAN;if(isfinite(t)&&t>=-100&&t<=150&&code>=0&&code<=255){n.temperatureTenths=(int16_t)lround(t*10);n.feelsLikeAvailable=isfinite(feels)&&feels>=-100&&feels<=150;if(n.feelsLikeAvailable)n.feelsLikeTenths=(int16_t)lround(feels*10);n.highLowAvailable=isfinite(high)&&isfinite(low)&&high>=-100&&high<=150&&low>=-100&&low<=150;if(n.highLowAvailable){n.highTenths=(int16_t)lround(high*10);n.lowTenths=(int16_t)lround(low*10);}n.humidityAvailable=humidity>=0&&humidity<=100;if(n.humidityAvailable)n.humidityPercent=(uint8_t)humidity;n.windAvailable=isfinite(wind)&&wind>=0&&wind<=500&&direction>=0&&direction<=360;if(n.windAvailable){n.windSpeedTenths=(uint16_t)lround(wind*10);n.windDirectionDegrees=(uint16_t)direction;}int dailyPrecip=dailyObject["precipitation_probability_max"][0]|precip;n.precipitationAvailable=dailyPrecip>=0&&dailyPrecip<=100;if(n.precipitationAvailable)n.precipitationPercent=(uint8_t)dailyPrecip;n.weatherCode=(uint8_t)code;n.dataAvailable=true;n.lastSuccessMs=now;n.state=State::Online;ok=true;lastRequestLatitude_=a;lastRequestLongitude_=o;lastRequestPositionValid_=true;}}}body=String();}http.end();}if(ok&&(n.source==LocationSource::Gps||n.source==LocationSource::Manual)&&(movedMaterially||!n.city[0]||!strcmp(n.city,"GPS LOCATION")||!strcmp(n.city,"MANUAL LOCATION")))resolveIdentity(n,a,o);if(!ok)n.state=n.dataAvailable&&now-n.lastSuccessMs<=appconfig::kWeatherFreshForMs?State::Stale:State::Offline;
